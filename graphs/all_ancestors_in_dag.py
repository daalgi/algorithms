...
def bfs(n: int, edges: List[List[int]]) -> List[List[int]]:
    ...
    # Adjacency matrix mapping { ancestor: children }
    adj = defaultdict(list)
    for ancestor, child in edges:
        adj[ancestor].append(child)
    ...
    for ancestor in range(n):
        ...
        # BFS to traverse the nodes starting from the current `ancestor`
        q = deque(adj[ancestor])
        while q:
            child = q.popleft()
            if not ans[child] or ans[child][-1] != ancestor:
                ans[child].append(ancestor)
                for grand_child in adj[child]:
                    q.append(grand_child)

    return ans


def bfs2(n: int, edges: List[List[int]]) -> List[List[int]]:
    # Time complexity: O(n²logn)
    # Space complexity: O(n²)
    # Sorting each subarray increases the time complexity
...
def bfs3(n: int, edges: List[List[int]]) -> List[List[int]]:
    ...
    # Start from the nodes with no ancestors (indegree = 0)
    # and go forward, update each child node with the ancestor
    # ancestors (avoid repeating computations of previous solutions).
    # Only add the next node when all its ancestors have been
    # visited (indegree = 0)
    while q:
        node = q.popleft()
